...
public class OptimisticLockTest extends IntegerTestSupport {
    ...
    public void issueCouponWithOptimisticTest() throws InterruptedException {
        ...
        for (User testUser : testUsers) {
            executorService.submit(() -> {
                try {
                    optimisticLockCouponFacade.issueCouponWithOptimistic(testUser.getId(), testCoupon.getId());
    ...
    public void issueCouponWithOptimisticFailTest() throws InterruptedException {
        ...
        for (User testUser : testUsers) {
            executorService.submit(() -> {
                try {
                    assertThrows(IllegalArgumentException.class, () ->  optimisticLockCouponFacade.issueCouponWithOptimistic(testUser.getId(), testCoupon.getId()));
                } finally {
                    latch.countDown();
                }
            });
